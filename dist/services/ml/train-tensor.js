...
async function loadTensorFlow() {
    try {
        ...
        console.warn('[ML] Falling back to @tensorflow/tfjs CPU backend because @tensorflow/tfjs-node failed to load', err);
...
export async function trainLinearDurationModelTF(limit = 1000) {
    ...
    const samples = rows.map(r => buildBaseAndExtraFeatures({
        pedido: r.pedido,
        tiempo: r,
    ...
    modelTF.add(tf.layers.dense({
        units: 1,
        inputShape: [nFeatures],
        useBias: false,
        kernelRegularizer: lambda && isFinite(lambda) && lambda > 0
    ...
    const yhatVaTensor = modelTF.predict(XvaTensor);
    const yhatVa = (await yhatVaTensor.array());
    const mae_valid = yhatVa.length === 0
        ? null
        : yhatVa.reduce((acc, row, i) => acc + Math.abs(row[0] - yva[i]), 0) / yhatVa.length;
    const yhatTrTensor = modelTF.predict(XtrTensor);
    const yhatTr = (await yhatTrTensor.array());
    ...
    const kernelArr = (await kernel.array());
    ...
    return { count: rows.length, path, model, mae: mae_valid, mae_train, mae_valid };
}

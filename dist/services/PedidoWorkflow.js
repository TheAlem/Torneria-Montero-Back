...
export async function transitionEstado(pedidoId, newEstado, opts = {}) {
    const now = new Date();
    ...
    if (newEstado === 'ENTREGADO') {
        ...
        try {
            ...
            logger.warn({ msg: '[PedidoWorkflow] Error evaluando semáforo', pedidoId, err: e?.message });
        }
    }
    ...
    try {
        ...
        if (newEstado === 'EN_PROGRESO' && !pedido.responsable_id) {
            try {
                await autoAssignIfEnabled(pedidoId);
            }
            catch { }
        }

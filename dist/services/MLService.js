...
export async function predictTiempoSec(pedidoId, trabajadorId) {
    ...
    const precioNum = typeof pedido.precio === 'object' || typeof pedido.precio === 'string' ? Number(pedido.precio) : (pedido.precio ?? 0);
    const descripcion = pedido.descripcion ?? null;
    ...
    {
        const modelPredDB = await predictWithLatestModel({ prioridad: pedido.prioridad, precio: precioNum, descripcion, workerSkills, cargaActual, fechaIngreso });
        if (modelPredDB)
            return Math.min(MAX_SEC, Math.max(MIN_SEC, modelPredDB));
        const modelPredFS = predictWithLinearModel({ prioridad: pedido.prioridad, precio: precioNum, descripcion, workerSkills, cargaActual, fechaIngreso });
        if (modelPredFS)
            return Math.min(MAX_SEC, Math.max(MIN_SEC, modelPredFS));
    }
    // Fallback por prioridad
    if (pedido.prioridad === 'ALTA')
        return Math.min(MAX_SEC, 3 * 60 * 60);
    if (pedido.prioridad === 'MEDIA')
        return Math.min(MAX_SEC, 6 * 60 * 60);
    return Math.min(MAX_SEC, 8 * 60 * 60);
}
export async function storePrediccion(pedidoId, trabajadorId, tEstimadoSec) {
    try {

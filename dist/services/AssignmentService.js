...
function prioridadBoost(pr) {
    return pr === 'ALTA' ? 1 : pr === 'MEDIA' ? 0.6 : 0.3;
}
export async function suggestCandidates(pedidoId) {
    ...
    for (const t of trabajadores) {
        ...
        const skillArr = normalizeSkills(t.skills);
        ...
        const skills = Array.isArray(t.skills) ? t.skills : skillArr;
    ...
    return out.sort((a, b) => {
        if (a.saturado !== b.saturado)
            return Number(a.saturado) - Number(b.saturado);
        if (b.score !== a.score)
...
export async function autoAssignIfEnabled(pedidoId) {
    if (String(process.env.AUTO_ASSIGN_ENABLED ?? 'false') !== 'true')
        return false;
    return autoAssignForced(pedidoId);
}
export async function autoAssignForced(pedidoId) {
    const candidates = await suggestCandidates(pedidoId);
    if (!candidates.length)
        return false;
    const choice = candidates.find(c => !c.saturado) || candidates[0];
    if (!choice || choice.saturado)
        return false;
    ...
    try {
        RealtimeService.emitToOperators('assignment:changed', { pedidoId, trabajadorId: choice.trabajadorId, ts: Date.now() });
    }
    catch { }
    return true;
}
export async function maybeReassignIfEnabled(pedidoId, color) {
    if (color !== 'ROJO')
        return false;
    if (String(process.env.AUTO_REASSIGN_ENABLED ?? 'false') !== 'true') {
        const candidates = await suggestCandidates(pedidoId);
        try {
            RealtimeService.emitToOperators('assignment:suggest', { pedidoId, candidates, ts: Date.now() });
        }
        catch { }
        return false;
    }
    ...
    if (current?.responsable_id === choice.trabajadorId)
        return false;

...
export const semanal = async (req, res, next) => {
    try {
        const now = new Date();
        ...
        const resumen = trabajos.reduce((acc, t) => { acc[t.estado] = (acc[t.estado] || 0) + 1; return acc; }, {});
...
export const mensual = async (req, res, next) => {
    try {
        const now = new Date();
...
export const alertas = async (req, res, next) => {
    try {

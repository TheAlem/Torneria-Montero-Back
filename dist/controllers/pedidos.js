...
export const listar = async (req, res, next) => {
    try {
        ...
        const where = onlyVisible ? { eliminado: false } : {};
...
export const getById = async (req, res, next) => {
    try {
        ...
        if (!pedido)
            return fail(res, 'NOT_FOUND', 'Pedido no encontrado', 404);
        // Ocultar eliminados salvo que lo pidan explícitamente
        const { includeDeleted } = req.query;
        const onlyVisible = String(includeDeleted || 'false').toLowerCase() !== 'true';
        if (onlyVisible && pedido.eliminado)
            return fail(res, 'NOT_FOUND', 'Pedido no encontrado', 404);
...
export const crear = async (req, res, next) => {
    try {
        ...
        try {
            RealtimeService.emitWebAlert('TRABAJO_AGREGADO', `Nuevo pedido #${created.id} creado`, { pedidoId: created.id, prioridad: pedido?.prioridad, cliente: pedido?.cliente?.nombre });
        }
        catch { }
        ...
        if (err?.name === 'ZodError')
            return fieldsValidation(res, err.errors ?? err);
        next(err);
    }
};
export const actualizar = async (req, res, next) => {
    try {
        ...
        if (!parsed.success)
            return fieldsValidation(res, parsed.error.flatten());
        ...
        if (typeof body.fecha_estimada_fin !== 'undefined')
            data.fecha_estimada_fin = body.fecha_estimada_fin ? new Date(body.fecha_estimada_fin) : null;
        if (typeof body.estado !== 'undefined')
            data.estado = body.estado;
        if (typeof body.responsable_id !== 'undefined')
            data.responsable_id = body.responsable_id;
        if (typeof body.semaforo !== 'undefined')
            data.semaforo = body.semaforo;
        if (typeof body.notas !== 'undefined')
            data.notas = body.notas;
        if (typeof body.adjuntos !== 'undefined')
            data.adjuntos = body.adjuntos;
        if (Object.keys(data).length === 0)
            return fail(res, 'VALIDATION_ERROR', 'No hay campos para actualizar', 400);
...
export const eliminar = async (req, res, next) => {
    try {
        ...
        if (err?.code === 'P2025')
            return fail(res, 'NOT_FOUND', 'Pedido no encontrado', 404);
        if (err?.code === 'P2003')
            return fail(res, 'CONFLICT', 'No se puede eliminar el pedido porque tiene registros asociados.', 409);
        next(err);
    }
};
export const cambiarEstado = async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const { estado, note, userId } = req.body;
        if (!estado)
            return fail(res, 'VALIDATION_ERROR', 'Debe indicar el nuevo estado', 400);

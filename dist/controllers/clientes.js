...
export const buscar = async (req, res, next) => {
    try {
        const { ci_rut, nombre, telefono } = req.query;
        ...
        if (ci_rut && ci_rut.trim()) {
            ...
            if (client)
                return success(res, { match: true, client });
...
export const crear = async (req, res, next) => {
    try {
        ...
        if (err?.code === 'P2002')
            return fail(res, 'UNIQUE_CONSTRAINT', 'Ya existe un registro con estos datos únicos.', 409);
        next(err);
    }
};
...
export const eliminar = async (req, res, next) => {
    try {
        ...
        if (err?.code === 'P2025')
            return fail(res, 'NOT_FOUND', 'Cliente no encontrado', 404);
        if (err?.code === 'P2003')
            return fail(res, 'CONFLICT', 'No se puede eliminar el cliente porque tiene registros asociados.', 409);
        next(err);
    }
};

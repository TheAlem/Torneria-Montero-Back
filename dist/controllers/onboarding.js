...
export const crearQR = async (req, res, next) => {
    try {
        const clienteId = Number(req.params.id);
        const cliente = await prisma.clientes.findUnique({ where: { id: clienteId } });
        if (!cliente)
            return fail(res, 'NOT_FOUND', 'Cliente no encontrado', 404);
...
export const validar = async (req, res, next) => {
    try {
        ...
        if (!entry)
            return fail(res, 'AUTH_ERROR', 'Token invÃ¡lido o expirado', 401);
        const { cliente } = entry;
        return success(res, {
...
export const completar = async (req, res, next) => {
    try {
        ...
        if (!password || String(password).length < 6)
            return fail(res, 'VALIDATION_ERROR', 'ContraseÃ±a invÃ¡lida', 400);
        const entry = await getValidToken(token);
        if (!entry)
            return fail(res, 'AUTH_ERROR', 'Token invÃ¡lido o expirado', 401);
        ...
        if (cliente.usuario_id)
            return fail(res, 'CONFLICT', 'Cliente ya estÃ¡ vinculado a un usuario', 409);
        ...
        if (!JWT_SECRET)
            return success(res, { profile: { id: user.id, email: user.email, role: user.rol } }, 201);
        const jwtToken = jwt.sign({ id: user.id, role: user.rol }, JWT_SECRET, { expiresIn: '8h' });

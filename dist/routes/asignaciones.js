...
router.get('/suggest', authenticate, requireRole('admin', 'tornero'), async (req, res, next) => {
    try {
        ...
        if (!Number.isFinite(pedidoId))
            return fail(res, 'VALIDATION_ERROR', 'pedidoId inválido', 422);
...
router.post('/auto', authenticate, requireRole('admin', 'tornero'), async (req, res, next) => {
    try {
        ...
        if (String(pedido.estado).toUpperCase() === 'ENTREGADO')
            return fail(res, 'VALIDATION_ERROR', 'El pedido ya fue entregado', 422);
        ...
        if (!done)
            return fail(res, 'CONFLICT', 'No se pudo auto-asignar (sin candidatos o saturados)', 409);

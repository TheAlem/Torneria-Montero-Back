...
export const semanal = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const trabajos = await prisma.job.findMany({ where: { dateCreated: { gte: lastWeek } }, include: { client: true, assignedWorker: true } });
...
export const mensual = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const trabajos = await prisma.job.findMany({ where: { dateCreated: { gte: lastMonth } }, include: { client: true, assignedWorker: true } });
    const resumen = trabajos.reduce((acc: any, t: any) => { acc[t.status] = (acc[t.status] || 0) + 1; return acc; }, {});
